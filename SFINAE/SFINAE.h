...
namespace SFINAE{
    ...
    // When looking for a best match for a set of arguments for a function templates, the compiler considers
    // whether the argument can be used in the way required by the complete function templates declaration
    ...
    void f(vector<int>&v, int* p,int n){
        ...
        //mean(p,p+n) with the first templates definition? The arguments match perfectly, but by substituting
        //in the actual templates argument ( int∗ ), we get the function declaration:
        ...
        //that such a substitution failure is not an error. It simply causes the templates to be ignored; that is,
        //the templates does not contribute a specialization to the overload set. That done, mean(p,p+n)
